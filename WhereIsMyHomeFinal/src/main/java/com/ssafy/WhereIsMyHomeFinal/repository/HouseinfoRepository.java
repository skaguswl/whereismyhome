...
public interface HouseinfoRepository extends JpaRepository<Houseinfo, Long> {
    ...
    @Query("select new com.ssafy.WhereIsMyHomeFinal.domain.dto.AptDto(h.apartmentName, h.lng, h.lat) from Houseinfo h where h.dongcode.id = :dongCode")
    List<AptDto> findApt(@Param("dongCode") String regcodePattern);
}
